...
# (4) Self-Consistency 평가 (Role/Task 유지, 출력만 Top-2 JSON) — "파싱하지 않고 원문 저장"
...
def retrieve_context_native(query: str, component_group: str, collection: chromadb.Collection, embed_model, ticket_to_keyword: dict = None):
...
def invoke_llm_with_token_tracking(prompt_template, input_data, step_name):
    ...
    return resp.content  # ★ 파싱하지 않음. 원문 그대로 반환.
...
chain = RunnableSequence(
    ...
    # Step 3: 제안 분리 및 전체 제안 요약 (영어 JSON 배열이 오더라도 파싱하지 않음)
    ...
    # Step 6: Self-Consistency 평가 (3회) — ★파싱 없이 원문 저장★
    RunnableLambda(timed("Self-Consistency 평가")(lambda x: x | {
        "self_consistency_responses": [
            invoke_llm_with_token_tracking(
                prompt_template=sc_prompt,
                input_data={
                    "proposal_summary": x["first_proposal"],
                    "top_10_table": x["top_10_table"]  # Role/Task 유지
                },
                step_name=f"Self-Consistency 평가 #{i+1}"
            ) for i in range(3)
        ],
        "component_group": x["component_group"]
    })),

    # (주의) Step 7/8: 파싱/표 생성 단계 제거 — 요청대로 파싱하지 않음

    # Step 9: 최종 결정 — ★파싱 없이 원문 저장★
    RunnableLambda(timed("최종 결정")(lambda x: x | {
        "final_result": invoke_llm_with_token_tracking(
            prompt_template=choose_prompt,
            input_data={
                "self_consistency_responses": json.dumps(x["self_consistency_responses"], ensure_ascii=False),
                "top_10_table": x["top_10_table"]  # Role/Task 유지
            },
            step_name="최종 결정"
        ),
...
def main(inputs):
    ...
    print("\n🧠 Self-Consistency (3회) 원문:")
    for i, resp in enumerate(result["self_consistency_responses"], start=1):
        print(f"\n[SC #{i} Raw]\n{resp}")
    ...
    # --- 반환 ---
    # 요청에 따라 SC/최종 결과를 "파싱하지 않은 원문"으로 그대로 반환.
    # (기존에 ticket_id/sim_score 등을 계산하던 로직은 파싱이 필요하므로 생략/빈값 반환)
    ticket_id = ""          # 파싱하지 않으므로 빈값
    sim_score = 0.0         # 파싱하지 않으므로 0.0
    ...
    return (
        ...
        after_change,
        generated_summary,
        sim_score,                    # 0.0 (파싱 미사용)
        "",                           # keyword (파싱 미사용 → 빈값)
        timing_results,
        proposal_summary_all,
        component_group,
        result.get("final_result", ""),                 # ★ 최종 결정 원문 문자열
        token_usage_results,
        icc_df.copy(),
        result.get("top_10_table", ""),
        result.get("top_10_records", []),
        result.get("self_consistency_responses", [])    # ★ SC 원문 문자열 리스트
    )
